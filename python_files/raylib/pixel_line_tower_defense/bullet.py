...
class Bullet:
    def __init__(self, spritesheet, sprite_dict, block_size, player_x, player_y) -> None:
    ...
    def draw(self):           
        for bullet in self.list:    
            p.draw_texture_pro(self.spritesheet,
                                (self.gun_flash[0], self.gun_flash[1], self.gun_flash[2], self.gun_flash[3]),
                                (self.gun_flash_x_window, self.gun_flash_y_window, self.block_size, self.block_size),
                                (0, 0),
                                0,
                                p.WHITE)
            p.draw_texture_pro(self.spritesheet,
