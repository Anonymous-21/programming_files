...
class Paddle:
    ...
    def draw(self):
        # draw left paddle
        r.DrawRectangle(self.left_x, self.left_y, self.width, self.height, self.color)
        # draw right paddle
        r.DrawRectangle(self.right_x, self.right_y, self.width, self.height, self.color)
    ...
    def move(self):
        ...
        if r.IsKeyDown(r.KEY_UP) and self.right_y >= 0:
            ...
            r.IsKeyDown(r.KEY_DOWN)
            and self.right_y <= r.GetScreenHeight() - self.height
        ):
            self.right_y += self.speed
